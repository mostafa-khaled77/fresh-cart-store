...
import { Loader2, MapPin, Phone, FileText, CheckCircle2, ArrowLeft, ShoppingBag } from "lucide-react";
...
import AddressCart from "../../AddressesAction/AddressCart/AddressCart";
import { Address } from "@/app/addresses/page";
import { Checkbox } from "@/components/ui/checkbox";
import { addAddress } from "@/app/_actions/address.actions";

export default function CheckoutCashForm({ initialData, savedAddresses }: { initialData: CartResponse | null, savedAddresses: any[] }) {
 ...
 const onSubmit = async (data: CheckoutFormValues) => {
    const finalCartId = cartData?.data?._id || cartData?.cartId;
    if (!finalCartId) {
    ...
    try {
      if (saveAddress) {
        try {
          ...
          console.error("Address saving failed, but continuing with order", addrErr);
        }
      }
      const res = await createCashOrder(finalCartId, data);
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
          <div className="lg:col-span-2">
            <div className="bg-white rounded-3xl shadow-sm border border-gray-100 overflow-hidden">
              ...
              <div className="p-8">
                <form id="cash-form" onSubmit={handleSubmit(onSubmit)} className="space-y-6">
                  <div className="space-y-1.5">
                    ...
                    <textarea
                      ...
                      className="w-full px-4 py-3 bg-gray-50 border border-gray-100 rounded-xl focus:border-yellow-400 focus:bg-white outline-none font-medium text-sm transition-all resize-none h-24"
                    />
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="space-y-1.5">
                      ...
                      <input {...register("phone")} type="tel" placeholder="01xxxxxxxxx" className="w-full px-4 py-3 bg-gray-50 border border-gray-100 rounded-xl focus:border-yellow-400 focus:bg-white outline-none font-medium text-sm transition-all" />
                    ...
                    <div className="space-y-1.5">
                      ...
                      <input {...register("city")} type="text" placeholder="Cairo..." className="w-full px-4 py-3 bg-gray-50 border border-gray-100 rounded-xl focus:border-yellow-400 focus:bg-white outline-none font-medium text-sm transition-all" />
            ...
            <div className="bg-white rounded-3xl shadow-xl overflow-hidden border border-gray-100">
              <div className="bg-slate-50 p-6 border-b border-gray-100">
                <h2 className="text-xl font-bold text-slate-900 flex items-center gap-2">
                  <ShoppingBag className="text-yellow-500" /> Order Summary
                </h2>
              </div>
              ...
              <div className="p-6">
                ...
                <button
                  type="submit" form="cash-form" disabled={isSubmitting}
                  className="w-full bg-yellow-400 text-slate-900 font-black py-4 rounded-xl mt-8 hover:bg-yellow-500 active:scale-[0.98] transition-all shadow-lg shadow-yellow-100 disabled:opacity-70 uppercase text-sm flex items-center justify-center gap-2"
                >
                  {isSubmitting ? <Loader2 size={18} className="animate-spin" /> : "Confirm Cash Order"}
                </button>
              </div>
            </div>
          </div>
