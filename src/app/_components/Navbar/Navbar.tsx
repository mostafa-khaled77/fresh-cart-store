...
import {
  ...
  Package, LogIn, UserPlus, Loader2,
  MapPin
} from "lucide-react";
      ...
      <nav className={`fixed top-0 left-0 right-0 z-[100] transition-all duration-300 px-6 md:px-12 ${
        ...
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          ...
          <div className="hidden lg:flex items-center gap-1 bg-white/50 rounded-full px-2 py-1 border border-slate-200">
            {["/", "/products", "/categories", "/brands"].map((link) => (
              <Link 
                key={link}
                href={link} 
                className={`px-5 py-2 rounded-full text-sm font-bold transition-all ${
                  path === link ? "bg-yellow-400 text-black shadow-sm" : "text-slate-600 hover:text-slate-900"
                }`}
              >
                {link === "/" ? "Home" : link.substring(1).charAt(0).toUpperCase() + link.substring(2)}
              </Link>
            ))}
          </div>
              ...
              <>
                <Link href="/wishlist" className="relative p-2 text-slate-700 hover:text-red-500 transition-colors">
                  ...
                  <span className="absolute top-0 right-0 bg-red-500 text-white text-[10px] min-w-[18px] h-[18px] flex items-center justify-center rounded-full border-2 border-white animate-in zoom-in duration-300">
                    {isWishLoading ? <Loader2 size={10} className="animate-spin" /> : (wishListCount || 0)}
                  </span>
                </Link>
                ...
                <Link href="/cart" className="relative p-2 text-slate-700 hover:text-yellow-600 transition-colors">
                  ...
                  <span className="absolute top-0 right-0 bg-red-500 text-white text-[10px] min-w-[18px] h-[18px] flex items-center justify-center rounded-full border-2 border-white animate-in zoom-in duration-300">
                    {isCartLoading ? <Loader2 size={10} className="animate-spin" /> : (cartCount || 0)}
                  </span>
                </Link>
              ...
              {["/", "/products", "/categories", "/brands"].map((link) => (
                <Link 
                  ...
                  className={`flex items-center justify-between p-4 rounded-2xl font-bold transition-all ${
                    path === link ? "bg-yellow-400 text-black shadow-lg shadow-yellow-100" : "hover:bg-slate-50 text-slate-600"
                  }`}
                >
                  <span>{link === "/" ? "Home" : link.substring(1).charAt(0).toUpperCase() + link.substring(2)}</span> 
